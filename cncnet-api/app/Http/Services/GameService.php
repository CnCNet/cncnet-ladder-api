...
class GameService
{
    ...
    /**
     * Temp copy paste of fillGameCols until we can workout what we're doing exactly.
     */
    public function fillDuneGameCols($game, $result)
    {
        $game->wol_game_id = $this->getUniqueGameIdentifier($result);
        foreach ($result as $key => $value)
        {
    ...
    /**
     * Temp copy paste of saveGameStats until we can workout what we're doing exactly.
     */
    public function saveDuneGameStats($result, $gameId, $playerId, $ladder, $cncnetGame)
    {
        $game = Game::where("id", "=", $gameId)->first();
        $player = Player::where("id", "=", $playerId)->first();
        ...
        foreach ($result as $key => $value)
        {
            ...
            if ($property == "PL_")
            {
                ...
                $playerHereName = explode("/", $value["value"])[0];
                $playerHere = Player::where('ladder_id', $ladder->id)->where('username', $playerHereName)->first();

...
class TeamMatchRequestTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // make a ladder
        $this->ladder = $this->makeLadder(4, true);
        $this->history = $this->makeLadderHistory($this->ladder);

    }

    private function matchMeUpRequest(Player $player, ?array $body = null) {

        $matchMeUpUrl = '/api/v2/qm/'.$this->ladder->abbreviation.'/';
    ...
    public function test_match_up(): void {
        ...
        $r1 = $this->matchMeUpRequest($p1);
        $r2 = $this->matchMeUpRequest($p2);
        $r3 = $this->matchMeUpRequest($p3);
        $r4 = $this->matchMeUpRequest($p4);

        Carbon::setTestNow($this->date->clone()->addSeconds(8));

        $r1 = $this->matchMeUpRequest($p1)->json();
        $r2 = $this->matchMeUpRequest($p2)->json();
        $r3 = $this->matchMeUpRequest($p3)->json();
        $r4 = $this->matchMeUpRequest($p4)->json();

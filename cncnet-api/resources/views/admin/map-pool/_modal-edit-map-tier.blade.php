<div class="modal fade" id="mapTierModal" tabindex="-1" role="dialog" aria-labelledby="mapTierModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                @foreach ($mapTiers as $mapTier)
                    <form method="POST" id="{{$mapTier->tier}}_addMapTierForm" val="{{$mapTier->tier}}" class="mapTierForm" action="editMapTier">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                        <input type="hidden" name="map_pool_id" value="{{ $mapPool->id }}" />

                        <div class="form-group">
                            <label for="tier">Map Tier</label>
                            <input type="number" class="form-control border" id="tier" name="tier" min=1 value="{{ $mapTier->tier }}">
                        </div>
                        <div class="form-group">
                            <label for="name">Map Tier Name</label>
                            <input type="text" class="form-control border" id="name" name="name" value="{{ $mapTier->name }}">
                        </div>
                        <div class="form-group">
                ...
                @foreach ($mapTiers as $mapTier)
                    <form method="POST" id="{{$mapTier->tier}}_deleteMapTierForm" val="{{$mapTier->tier}}" class="mapTierForm" action="deleteMapTier">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                        <input type="hidden" name="map_pool_id" value="{{ $mapPool->id }}" />

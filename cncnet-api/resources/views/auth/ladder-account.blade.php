                            ...
                            <div class="clan-listings">
                                ...
                                @foreach($clanPlayers as $clanPlayer)
                                    <div class="clan-listing active">
                                        <div class="clan-name clan-name-short">
                                            <a href="/clans/{{ $ladder->abbreviation }}/edit/{{ $clanPlayer->clan_id }}/main"><h2>{{ $clanPlayer->clan->short }}</h2></a>
                                        </div>
                                        <div class="clan-name clan-name-long">
                                            <a href="/clans/{{ $ladder->abbreviation }}/edit/{{ $clanPlayer->clan_id }}/main"><h2>{{ $clanPlayer->clan->name }}</h2></a>
                                        </div>
                                        <div class="text-center">
                                            <p>{{ $clanPlayer->role }}<strong class="username">{{ $clanPlayer->player->username }}</strong></p>
                                        </div>
                                ...
                                @foreach($invitations as $invite)
                                    <div class="clan-listing">
                                        ...
                                        <div>
                                            <p>Has invited <strong class="username">{{ $invite->player->username }}</strong></p>
                                        </div>
                                        <div class="clan-username">
                ...
                <div class="account-box">
                    ...
                    <div class="account-player-listings">
                        ...
                        @foreach($players as $player)
                            <div class="player-listing {{ $activeHandles->where('player_id', $player->id)->count() > 0 ? 'active': ''}}">
                                <div class="username">
                                    ...
                                    <form id="playerCard" class="form-inline" method="POST" action="card">
                                        ...
                                        <select class="form-control" name="cardId">
                                            @foreach($cards as $card)
                                                <option value="{{ $card->id }}" @if($card->id == $player->card_id) selected @endif>
                                                    {{ $card->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="username-status">
                                    <p>
                                        {{
                                        $activeHandles->where('player_id', $player->id)->count() > 0
                                        ?

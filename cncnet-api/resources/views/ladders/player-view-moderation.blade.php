@if ($userIsMod)
    <div>
        <a href="/admin/moderate/{{ $ladderId }}/player/{{ $ladderPlayer->id }}" class="btn btn-md btn-warning">Moderation Actions</a>
    </div>
@endif

@if (isset($mod) && $mod->isLadderAdmin($player['ladder']))
    <button type="button" class="btn btn-warning btn-md mt-2" data-bs-toggle="modal" data-bs-target="#submitLaundryService">Laundry Service</button>

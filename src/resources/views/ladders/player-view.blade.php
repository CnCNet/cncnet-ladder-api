...
@section('content')
    <?php $card = \App\Models\Card::find($ladderPlayer->player->card_id); ?>
                ...
                <div class="player-header">
                    <div class="player-stats">
                        ...
                        <ul class="list-inline text-uppercase">
                            ...
                            @if ($userIsMod)
                                <li>
                                    <a href="/admin/moderate/{{ $ladderId }}/player/{{ $ladderPlayer->id }}"
                                        class="btn btn-sm btn-danger">Moderation Actions</a>
                                </li>
                            @endif

                            @if (isset($mod) && $mod->isLadderAdmin($player['ladder']))
                                <button type="button" class="btn btn-danger btn-sm" data-toggle="modal"
                                    data-target="#submitLaundryService">Laundry Service</button>
                                                    ...
                                                    <div class="row content">
                                                        <div class="col-md-12 player-box player-card list-inline">
                                                            ...
                                                            @if (!$player->laundered($history))
                                                                <label>Are you sure you want to set all of
                                                                    {{ $player->username }}'s points to 0?</label>
                                                            @endif

                                                            <div style="display: inline-block">
                                                                <form method="POST"
                                                                    action="/admin/moderate/{{ $player->ladder->id }}/player/{{ $player->id }}/laundry">
                                                                    <input type="hidden" name="_token"
                                                                        value="{{ csrf_token() }}">
                                                                    <input name="player_id" type="hidden"
                                                                        value="{{ $player->id }}" />
                                                                    <input name="ladderHistory_id" type="hidden"
                                                                        value="{{ $history->id }}" />
                                                                    <button type="submit" name="submit" value="update"
                                                                        class="btn btn-danger btn-md">Launder</button>
                                                                </form>
                                                            </div>
                                                                ...
                                                                <div style="padding-top: 5px; display: inline-block">
                                                                    <form method="POST"
                                                                        ...
                                                                        <input name="ladderHistory_id" type="hidden"
                                                                            value="{{ $history->id }}" />
                                                                        <button type="submit" name="submit" value="update"
                                                                            class="btn btn-primary btn-sm">Undo
                                                                            Launder</button>
                                                                    </form>
                                                                </div>
                                                            @endif
                        ...
                        <div class="achievements-list">
                            @if ($ladderPlayer->game_count >= 200)
                                <div class="achievement">
                                    ...
                                    <h5 style="font-weight: bold; text-transform:uppercase; font-size: 10px;">Played
                                        <br />200+ Games
                                    </h5>
                                </div>
                            @endif
                            ...
                            @if ($ladderPlayer->rank <= 10)
                                <div class="achievement">
                                    ...
                                    @elseif($ladderPlayer->rank > 5 && $ladderPlayer->rank <= 10)
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        @include('components.player-recent-games', [
                            'player' => $ladderPlayer,
                            'games' => $games,
                        ])

                    ...
                    @foreach($heapNames as $heapName)
                        <div class="heap-panel" style="margin-bottom: 0;border: 1px solid;background: black;">
                            <div class="panel-header" id="headingFor{{ $heapName->name }}">
                                <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#accordianFor{{ $heapName->name }}" aria-expanded="true" aria-controls="accordianFor{{ $heapName->name }}">
